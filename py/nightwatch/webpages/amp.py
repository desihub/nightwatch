...
from bokeh.models import (
    ...
    NumeralTickFormatter, BasicTicker, AdaptiveTicker,
...
def plot_amp_qa(data, name, lower=None, upper=None, amp_keys=None, title=None, plot_height=80, plot_width=700, ymin=None, ymax=None):
    ...
    for cam in ['B', 'R', 'Z']:
        ...
        if name == "READNOISE":
            ...
            fig.yaxis.ticker = BasicTicker(desired_num_ticks=5, mantissas=[1], min_interval=1)
        if name == "BIAS":
            fig.yaxis.formatter = NumeralTickFormatter(format='e')
            fig.yaxis.ticker = AdaptiveTicker(desired_num_ticks=5, base=10, mantissas=[1, 1.5, 2, 2.5, 5])
        if name == "COSMICS_RATE":
            fig.yaxis.formatter = NumeralTickFormatter(format='a')
            fig.yaxis.ticker = AdaptiveTicker(desired_num_ticks=5, base=10, mantissas=[1, 2, 3, 4, 5])
