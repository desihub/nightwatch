...
def calcnominalnoise(nightwatchdir, nightexpids, outfile):
    ...
    TODO:
      ...
      * directly write a READNOISE-*.json file instead of an intermediate FITS file
    """
    ...
    #- then do something to convert to nominal min/max values to use for thresholds
    #- ...

    #----- end EXAMPLE

    #-----
    #- previous code, to be updated as needed
    ...
    fitsio.write(outfile, rnnoms)
    
if __name__ == "__main__":
    """
    e.g.
    python calcnominalreadnoise.py --indir /global/cfs/cdirs/desi/spectro/nightwatch/nersc \
        --nightexpids 20210107/00071175 20210108/00071353 20210109/00071521 20210110/00071664 20210111/00071802 \
        --outfile READNOISE-20210107.json
    """
    import argparse

    parser = argparse.ArgumentParser(usage = "{prog} [options]")
    parser.add_argument("--indir", type=str,  help="base directory with nightwatch processed data")
    parser.add_argument("--nightexpids", type=str, nargs="*", help="input night/expids to use for readnoise")
    parser.add_argument("--outfile", type=str,  help="output json threshold file to write")
    args = parser.parse_args()

    #- TODO: it might be easier for --nightexpids to be a text file that contains
    #- the night/expids to use, one per line, instead of trying to specify N>>1
    #- night/expids from a command line

    #- convert input nightexpids strings to list of (night,expid) tuples
    nightexpids = list()
    for tmp in args.nightexpids:
        n, e = tmp.split('/')
        nightexpids = int(n), int(e)
